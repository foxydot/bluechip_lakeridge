...
add_action('admin_enqueue_scripts','msdlab_kyaneos_options_scripts');
function msdlab_kyaneos_options_scripts(){
...
function msdlab_kyaneos_settings_box() {
    ...
    if (isset($_REQUEST['file'])) { 
        check_admin_referer("kyaneos_options");
...
$modal_update_href = esc_url( add_query_arg( array(
    'page' => 'genesis',
    '_wpnonce' => wp_create_nonce('kyaneos_options'),
), admin_url('admin.php') ) );
    if($logo != ''){
...
add_action('wp_head','msdlab_kyaneos_logo');
function msdlab_kyaneos_logo(){
    if(!is_admin()){
